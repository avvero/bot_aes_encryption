...
appender("flow", MarkerSocketAppender) {
    ...
    port = 4561
    marker = "bot_aes_encryption"
}

appender("file", RollingFileAppender) {
    file = "/var/log/service/bot_aes_encryption/service.log"
    encoder(PatternLayoutEncoder) {
        charset = java.nio.charset.StandardCharsets.UTF_8
        pattern = "%d{yyyy-MM-dd HH:mm:ss.SSS} %5p %t %c{0}:%M:%L - %m%n"
    }
    rollingPolicy(TimeBasedRollingPolicy) {
        fileNamePattern = "/var/log/service/bot_aes_encryption/service.log.%d{yyyy-MM-dd}"
    }
}
